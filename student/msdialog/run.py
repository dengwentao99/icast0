...
def tsallis_entropy_2D(input_tensor, alpha):
    return ((1 / (alpha - 1)) * (1 - (input_tensor ** alpha))).sum(1).reshape(-1, 1)


def tsallis_entropy_3D(input_tensor, alpha):
    return ((1 / (alpha - 1)) * (1 - (input_tensor ** alpha))).sum(2)


def information_gain_score(input_tensor):
    alpha = 1.5
    mean_logits = input_tensor.mean(dim=0)
    std_logits = ((input_tensor - mean_logits) ** 2).mean(0)
...
def _predict_batch(args, model, test_answer_dataloader, prefix,
    ...
    for step, batch in enumerate(test_answer_dataloader):
        ...
        with torch.no_grad():
            ...
            for t in range(T):
                output = model(**inputs)
                mc_loss, mc_reward_logits, sample_reward_output = output[:3]
            ...
            std_logits = -(mean_mc_logits * torch.log(mean_mc_logits)).sum(1).reshape(-1, 1)
            norm = torch.normal(torch.tensor(0.0), torch.tensor(1.0))
            model.eval()
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        checkpoints = [os.path.join(eval_output_dir, 'checkpoint-feedback-33700'),
                       os.path.join(eval_output_dir, 'checkpoint-feedback-34100')]
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        checkpoints = [os.path.join(eval_output_dir, 'checkpoint-answer-' + str(idx)) for
                       idx in range(215000, 300000, 5000)]
        if args.eval_all_checkpoints:
            checkpoints = list(
    ...
    if args.do_predict2 and args.local_rank in [-1, 0]:
        ...
        checkpoints = [os.path.join(eval_output_dir, 'warmup')]
        if args.eval_all_checkpoints:
            checkpoints = list(
                os.path.dirname(c) for c in
                sorted(glob.glob(args.model_name_or_path + "/**/" + WEIGHTS_NAME, recursive=True))
